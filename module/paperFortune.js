	...
	controller.hears('おみくじ',['direct_message','direct_mention','mention','ambient'],function(bot,message) {
		if(message.user == maccoto.id) {
			bot.reply(message, mtRandom([
				...
				// '*鶏* 今日のラッキーサラダチキンは' + getRandom(['プレーン', 'スモーク', 'シトラスレモン', 'ハーブ', 'ガーリックペッパー', 'タンドリー', 'サラチキバー', 'バジルサラチキバー']) + 'だよ！',
				'*筋肉* 今日のラッキープロテインは' + getRandom(['ミルクティー', 'ブルーベリーチーズケーキ', 'かめはめ波風味']) + 'だよ！',
				'*テイン* 今日は' + getRandom(['6レイヤーバー', 'プロテインウエハー', 'プロテインブラウニー']) + '食べたいねー。',
			...
			bot.reply(message, mtRandom([
				...
				'*大当* 大当たりー！今日は筋トレ休んでもいいよ！',
				'*中当* 今日の筋トレは' + getRandom(['1', '2']) + 'セットまででいいよー！',
				'*筋肉* プロテインをしっかり飲めば、肌艶がよくなるよ！',
				'*凶* :scream:',
				'*欲* お肉が食べたくなーる。美味しいお肉が食べたくなーる。',
				'運勢、大丈夫！今忙しいっ！じゃっ！:risu::dash:',
			], chimpkinDate_Y + chimpkinDate_M + chimpkinDate_D))
		}
	})
